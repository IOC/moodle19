YAHOO.util.Event.onDOMReady(function(){
    ...
    var url = 'index.php?overview=1&t=' + new Date().getTime();
    YAHOO.util.Connect.asyncRequest('GET', url, { success: handleSuccess });
});

var getremotecourses = function(){
    YAHOO.util.Event.onDOMReady(function(){
        var url = M_config['url'] + '?t=' + new Date().getTime();
        YAHOO.util.Connect.asyncRequest('GET', url, { success: myremotecoursescallback });
    });
};

var myremotecoursescallback = function(response){
    var Y = YAHOO.util.Dom;
